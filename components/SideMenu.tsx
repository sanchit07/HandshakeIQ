...
const SideMenu: React.FC<SideMenuProps> = ({ isOpen, onClose, history, dossiers, onSelectPerson, onRefreshPerson, onGoToSettings, onGoToUpcomingMeetings, onLogout }) => {
    ...
    return (
        <>
            <div 
                className={`fixed inset-0 bg-black/60 z-[80] transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                onClick={onClose}
            ></div>
            <aside className={`fixed top-0 right-0 h-full w-[280px] sm:w-[320px] bg-gray-900/95 backdrop-blur-lg border-l border-cyan-500/20 z-[90]
