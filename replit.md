...
- **`server/jobs/atsSubmitter/core.ts`**: defense-in-depth — a password/passphrase-labeled field now classifies as sensitive with its own dedicated pause message that never mentions Screening Answers, regardless of how it's reached (never AI-drafted, never vault-matched, never vault-written).
- New `server/jobs/atsSubmitter/sessionExpiry.test.ts` (`npm run test:session-expiry`, real Chromium + DB fixture) proves a mid-wizard login page gets signed back in with the exact vaulted password rather than paused/filled; new `core.test.ts` unit tests cover the classification/pause behavior.
